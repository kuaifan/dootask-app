package eeui.android.eeuiNotifications.utils;
...
public class NotificationUtils extends ContextWrapper {
     ...
     * @param title
     */
    public void clearNotificationTitle(String title) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            StatusBarNotification[] lists = getManager().getActiveNotifications();
            for (StatusBarNotification item : lists) {
                try {
                    if (item.getPackageName().equals(getPackageName())) {
                        Bundle mBundle = item.getNotification().extras;
                        String mTitle = null;
                        for (String key : mBundle.keySet()) {
                            if (key.endsWith("title")) {
                                mTitle = mBundle.getString(key);
                            }
                        }
                        if (mTitle != null && mTitle.equals(title)) {
                            clearNotificationId(item.getId());
                        }
                    }
